...
# Built ONCE at module load time — shared across all connections
# ==============================
def build_detector():
    base_options = python.BaseOptions(
        model_asset_path="hand_landmarker.task"
...
# Singleton detector — created once, reused by all ASLRecognizer instances
_shared_detector = build_detector()


class ASLRecognizer:
    def __init__(self, model, labels, detector=None):
        self.model = model
        self.labels = labels

        # Use shared detector if none provided — saves ~50MB per connection
        self.detector = detector if detector is not None else _shared_detector
        ...
        print("✅ ASL Recognizer Initialized (shared detector)")
    ...
    # ==============================
    def run_prediction(self):
        # Build tensor — use float32 for CPU inference stability
        x = torch.tensor(
            np.array(list(self.sequence)),
            dtype=torch.float32
        ).unsqueeze(0)
    ...
    def process_frame(self, frame):
        ...
        # Increment by 66ms to match effective ~15fps after skip
        self.frame_timestamp += 66
